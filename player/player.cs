...
public class player : KinematicBody2D
{
	...
	PackedScene swordSlash;
	AudioStreamPlayer audioattack;
	AudioStreamPlayer audiohurt;
	private Node Globals;
	private Node Hud;
	// Called when the node enters the scene tree for the first time.
	public override void _Ready()
	{
		swordSlash = (PackedScene)ResourceLoader.Load("res://player/swordbeam/swordbeam.tscn");
		...
		audioattack = GetNode<AudioStreamPlayer>("swipe");
		audiohurt = GetNode<AudioStreamPlayer>("hurt");

		Hud = GetNode("/root/Hud");
		Globals = GetNode("/root/Global");
		GD.Print(Globals.Get("score"));
	}
	...
	private void run()
	{
		...
		bool sound = (bool)Globals.Get("Sound");
	...
	private void shoot()
	{
		...
		{
			righthurtbox.Disabled = false;
			Area2D boule = (Area2D)swordSlash.Instance();
			AddChild(boule);
			boule.GlobalPosition = GlobalPosition;
			boule.Rotation = 0;
		}
		else
		{
	...
	public override void _PhysicsProcess(float delta)
	{
		...
		Globals.Set("playerposition", GlobalPosition);

		if (Input.IsActionPressed("ui_sound") && !disabled)
		{
			disabled = true;
			if((bool)Globals.Get("Sound"))
			{
				Globals.Set("Sound", false);
			}
			else
			{
	...
	private void die()
	{
		...
		GetTree().ChangeScene("res://menus/GameOver.tscn");
